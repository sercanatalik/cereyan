...
| Multi-user accounts, roles, SSO, audit trails | One token, one permission level, one machine. | Put a [reverse proxy](../guides/secure-the-server.md#put-it-behind-nginx-at-a-sub-path) in front for network access; use the token or the socket. |
| Cloud-style extras: assets, SLAs, incident management, metric triggers, incoming webhooks as event sources | Each is a product on its own; rules with `unless` and custom routes cover the local versions of the common cases. | A proactive rule for "did not happen by"; `fresh_within`, `expect_by` and `expected_duration` on a flow as shorthand, shown as PASS, WARN or FAIL; a custom route that calls `emit_event` for incoming webhooks. |
