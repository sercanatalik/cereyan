...
- Cancelling a run moves it to `Cancelling`, tells the engine, waits `cancel_grace_secs`, sends SIGTERM, waits again, then SIGKILL. The engine raises `KeyboardInterrupt` in the flow so it can clean up. It reaches Python code and `time.sleep` on every platform; on Windows it cannot reach most blocking I/O, so a flow waiting there runs until the supervisor ends the process, which Windows does abruptly in place of SIGTERM.
