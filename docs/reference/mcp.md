...
| `get_run` | `run_id` (integer, required) | One run with its state, parameters, timing, and task runs. Response keys: `run`, `task_runs`, `task_state` |
...
| Tool | Arguments | Returns |
|---|---|---|
| `run_flow` | `at` (string) — Start at this time, ISO 8601, instead of now<br>`delay_seconds` (number) — Start this many seconds from now; not with at<br>`flow` (string, required) — Flow name, or project/flow when the name exists in several projects<br>`name` (string) — Optional run name<br>`parameters` (object) — Flow parameters as JSON<br>`tags` (array of string) | Start a run of a flow now, or at a later time with `at` or `delay_seconds`. Creates the run immediately and returns without waiting; follow it with get_run. Parameters are validated against the flow's schema. Response keys: `conflict`, `note`, `run` |
...
| `backfill` | `concurrency` (integer, default `1`)<br>`dry_run` (boolean, default `true`)<br>`end` (string)<br>`extra_parameters` (object)<br>`flow` (string, required) — Flow name, or project/flow when the name exists in several projects<br>`force` (boolean) — Restate: the runs ignore targets, caches, checkpoints and bulk_complete<br>`interval` (string) — Seconds or a duration such as 1d or 12h (default 1d)<br>`missing_only` (boolean) — Leave out values whose latest run completed<br>`parameter` (string, required)<br>`reverse` (boolean)<br>`start` (string) — YYYY-MM-DD or RFC 3339; not needed with values<br>`values` (array of string) — Explicit parameter values instead of a range | Create one run per value of a date or datetime parameter between start and end. Defaults to a dry run that only reports how many runs would be created; pass dry_run false to create them. Can create thousands of runs. Response keys: dry run: `dry_run`, `first`, `flow`, `interval_seconds`, `last`, `note`, `parameter`, `runs`<br>dry_run false: `backfill`, `dry_run` |
