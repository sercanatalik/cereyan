...
The Unix socket listener and engine niceness are Unix-only options, rejected or ignored with a message. Three behaviours are weaker on Windows, all of them signal-based:
...
| A clock-armed proactive rule (`unless` with `at`) | Can lapse while the events it waits for are still arriving. |
