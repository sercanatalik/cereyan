...
Unique keys with debounce, resources keyed by parameter, and runs created for later.

Source: [`examples/admission.py`](https://github.com/sercanatalik/cereyan/blob/main/examples/admission.py). Run it with `python examples/admission.py` while `cereyan serve examples/` is running.
