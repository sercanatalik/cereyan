...
A task remembers the job it started, a durable sleep frees the engine, and a retry replays the finished prefix.

Source: [`examples/durable_load.py`](https://github.com/sercanatalik/cereyan/blob/main/examples/durable_load.py). Run it with `python examples/durable_load.py` while `cereyan serve examples/` is running.
