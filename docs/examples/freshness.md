...
Health per flow from its runs, a rule that pages when a run overruns, and a backfill of just the missing days.

Source: [`examples/freshness.py`](https://github.com/sercanatalik/cereyan/blob/main/examples/freshness.py). Run it with `python examples/freshness.py` while `cereyan serve examples/` is running.
